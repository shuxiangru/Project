...
summary(model1b)
extractAIC(model1b)
plotres(model1b, which=3:4, caption = "Forward selected full interaction \n model assumption check")


#Forward selected full interaction model cv
forward_sel_intr_model_cv = train(formula(model1b), data=df_adult, method="lm", preProcess="scale", trControl=train_control)
...
model2 = lm(update.formula(base_form, .~.+famsize*(famincome+log.grocery+log.eatout+log.delivery)), data = df_adult)
summary(model2)
plotres(model2, which=3:4, caption = "Base model plus family interaction \nterm assumption check")
```
...
```
```{r}
# best model (model1b) bootstraping 
...
nsim = 1000
results <- boot(data=df_adult, statistic=bs, R=nsim, formula=formula(model1b))
names_ = names(coef(lm(formula(model1b), data=df_adult)))
...
multinomial_sel_intr_form =  update.formula(formula(model1b), bmi_class~.)
