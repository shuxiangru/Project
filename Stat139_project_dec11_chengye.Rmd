...
base_sleep_form = update.formula(base_form, . ~. + I(sleep_hr^2))

model_sleep = lm(base_sleep_form, data = df_adult)
summary(model_sleep)
plotres(model_sleep, which=3:4, caption = "Base model plus sleephour^2 assumption check")

#sleep model cv
model_sleep_cv = train(base_sleep_form, data=df_adult, method="lm", preProcess="scale", trControl=train_control)

print("Base model plus sleephour^2 cv results:")
print(paste("RMSE:", model_sleep_cv$results$RMSE, "Rsquared:", model_sleep_cv$results$Rsquared))
...
summary(model1a)
extractAIC(model1a)
plotres(model1a, which=3:4, caption = "Backward selected sleep model\n assumption check")


#model1a cv
model1a_cv = train(formula(model1a), data=df_adult, method="lm", preProcess="scale", trControl=train_control)
...
full_intr_form = update(base_sleep_form, .~ (.)^2)
model_interact = lm(full_intr_form, data = df_adult)
summary(model_interact)
plotres(model_interact, which=3:4, caption = "Full interaction model assumption check")

#full interaction model cv
model_interact_cv = train(full_intr_form, data=df_adult, method="lm", preProcess="scale", trControl=train_control)

print("Full interaction model cv results:")
print(paste("RMSE:", model_interact_cv$results$RMSE, "Rsquared:", model_interact_cv$results$Rsquared))

```

```{r fig.height=3, fig.width=3, fig.align="center"}
# Based on model1, add interaction terms between famsize and (famincome, log.grocery, log.eatout and log.delivery)
model1b = lm(update.formula(base_form, .~.+famsize*(famincome+log.grocery+log.eatout+log.delivery)), data = df_adult)
summary(model1b)
plotres(model1b, which=3:4, caption = "Base model plus family interaction \nterm assumption check")
```


```{r fig.height=3, fig.width=3, fig.align="center", warning=FALSE, echo=FALSE}
#forward selection from intercept only model to full model
model1c = step(lm(log2.bmi ~ 1, data = df_adult), scope = list(upper = model_interact), direction = "forward", trace = 0)
summary(model1c)
extractAIC(model1c)
plotres(model1c, which=3:4, caption = "Forward selected full interaction \n model assumption check")


#Forward selected full interaction model cv
forward_sel_intr_model_cv = train(formula(model1c), data=df_adult, method="lm", preProcess="scale", trControl=train_control)

print("Forward selected full interaction model cv results:")
print(paste("RMSE:", forward_sel_intr_model_cv$results$RMSE, "Rsquared:", forward_sel_intr_model_cv$results$Rsquared))
...
```
```{r}
# best model (model1c) bootstraping 
...
nsim = 1000
results <- boot(data=df_adult, statistic=bs, R=nsim, formula=formula(model1c))
names_ = names(coef(lm(formula(model1c), data=df_adult)))
...
multinomial_sel_intr_form =  update.formula(formula(model1c), bmi_class~.)

multinomial_sel_intr_cv = train(multinomial_sel_intr_form, data=df_adult, method="multinom", preProcess="scale", trControl=train_control, trace=0)

print(paste("Multinomial classification forward selected full interaction model cv results:", max(multinomial_sel_intr_cv$results$Accuracy)))
