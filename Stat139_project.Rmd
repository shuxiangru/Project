...
```{r fig.height=3, fig.width=3, fig.align="center"}
# add sleep^2 to basemodel
df_adult["sleep_hr2"] = (df_adult$sleep_hr)^2
model_sleep = lm(log2.bmi ~ gender+age+race+edu+marriage+famsize+famincome+alcohol12yr+alcoholfrq+log.grocery+log.eatout+log.delivery+milk+meals_nothome+meals_fastfood+depressed+sleep_trouble+activity+tv_hrs+sleep_hr+smoke + sleep_hr2, data = df_adult)
summary(model_sleep)
check_assumptions(model_sleep)
```

```{r fig.height=3, fig.width=3, fig.align="center"}
#backward selection from model_sleep
model1a = step(model_sleep, trace = 0, direction = "backward")
summary(model1a)
extractAIC(model1a)
check_assumptions(model1a)
```

```{r fig.height=3, fig.width=3, fig.align="center"}
#full model with all the 2 way interactions(including sleep_hr2)
model_interact = lm(log2.bmi ~ (gender+age+race+edu+marriage+famsize+famincome+alcohol12yr+alcoholfrq+log.grocery+log.eatout+log.delivery+milk+meals_nothome+meals_fastfood+depressed+sleep_trouble+activity+tv_hrs+sleep_hr+smoke + sleep_hr2)^2, data = df_adult)
summary(model_interact)
check_assumptions(model_interact)
```


```{r fig.height=3, fig.width=3, fig.align="center"}
#forward selection from intercept only model to full model
model1b = step(lm(log2.bmi ~ 1, data = df_adult), scope = list(upper = model_interact), direction = "forward", trace = 0)
summary(model1b)
extractAIC(model1b)
check_assumptions(model1b)
```

```{r fig.height=3, fig.width=3, fig.align="center"}
# Based on model1, add interaction terms between famsize and (famincome, log.grocery, log.eatout and log.delivery)
model2 = lm(log2.bmi~gender+age+race+edu+marriage+famsize+famincome+alcohol12yr+alcoholfrq+log.grocery+log.eatout+log.delivery+milk+meals_nothome+meals_fastfood+depressed+sleep_trouble+activity+tv_hrs+sleep_hr+smoke + famsize*famincome + famsize*log.grocery+famsize*log.eatout + famsize*log.delivery, data = df_adult)
summary(model2)
...
x.lasso = model.matrix(formula(model_interact), data=df_adult)
model_lasso = cv.glmnet(x.lasso, df_adult$log2.bmi, nfolds = 5)
#plot
plot(model_lasso)
lasso.lambda.min = model_lasso$lambda.min
lasso.mse.min = min(model_lasso$cvm)

coef.lasso = coef(model_lasso, s = "lambda.min")
nonzero_coef_lasso = cbind(rownames(coef.lasso)[which(coef.lasso != 0)], coef.lasso[which(coef.lasso != 0)])

```
```{r}
#ridge
x.ridge = model.matrix(formula(model_interact), data=df_adult)
model_ridge = cv.glmnet(x.ridge, df_adult$log2.bmi, alpha = 0, nfolds = 5)
#plot
plot(model_ridge)
ridge.lambda.min = model_ridge$lambda.min
ridge.mse.min = min(model_ridge$cvm)

coef.ridge = coef(model_ridge, s = "lambda.min")
nonzero_coef_ridge = cbind(rownames(coef.ridge)[which(coef.ridge != 0)], coef.ridge[which(coef.ridge != 0)])
```

```{r}
#elastic net
x.mix = model.matrix(formula(model_interact), data=df_adult)
model_mix = cv.glmnet(x.mix, df_adult$log2.bmi, alpha = 0.5, nfolds = 5)
#plot
plot(model_mix)
mix.lambda.min = model_mix$lambda.min
mix.mse.min = min(model_mix$cvm)

coef.mix = coef(model_mix, s = "lambda.min")
nonzero_coef_mix = cbind(rownames(coef.mix)[which(coef.mix != 0)], coef.mix[which(coef.mix != 0)])
```
...
# model sleep
msleep_cv = train(log2.bmi ~ gender + age + race + edu + marriage + famsize + famincome + alcohol12yr + alcoholfrq + log.grocery + log.eatout + log.delivery + milk + meals_nothome + meals_fastfood + depressed + sleep_trouble + activity + tv_hrs + sleep_hr + smoke + sleep_hr2
, data=df_adult, method="lm", preProcess="scale", trControl=train_control)

print("model_sleep cv results:")
print(msleep_cv$results)



# model 1a
m1a_cv = train(log2.bmi ~ gender + race + edu + marriage + famsize + famincome + alcoholfrq + log.eatout + milk + depressed + activity + tv_hrs + sleep_hr + smoke + sleep_hr2
, data=df_adult, method="lm", preProcess="scale", trControl=train_control)

print("model_1a cv results:")
print(m1a_cv$results)


# model interact
mintr_cv = train(log2.bmi ~ (gender + age + race + edu + marriage + famsize + famincome + alcohol12yr + alcoholfrq + log.grocery + log.eatout + log.delivery + milk + meals_nothome + meals_fastfood + depressed + sleep_trouble + activity + tv_hrs + sleep_hr + smoke + sleep_hr2)^2
, data=df_adult, method="lm", preProcess="scale", trControl=train_control)

print("model_interact cv results:")
print(mintr_cv$results)

# model 1b
m1b_cv = train(log2.bmi ~ race + tv_hrs + smoke + depressed + famincome + milk + gender + marriage + log.eatout + sleep_hr + sleep_hr2 + alcoholfrq + activity + meals_fastfood + famincome:gender + smoke:gender + race:gender + smoke:marriage + depressed:famincome + race:activity + depressed:milk + log.eatout:sleep_hr + race:meals_fastfood + gender:meals_fastfood + tv_hrs:log.eatout + smoke:sleep_hr2 + sleep_hr:alcoholfrq
, data=df_adult, method="lm", preProcess="scale", trControl=train_control)

print("model_1b cv results:")
print(m1b_cv$results)


# model 2
m2_cv = train(log2.bmi ~ gender + age + race + edu + marriage + famsize + famincome + alcohol12yr + alcoholfrq + log.grocery + log.eatout + log.delivery + milk + meals_nothome + meals_fastfood + depressed + sleep_trouble + activity + tv_hrs + sleep_hr + smoke + famsize * famincome + famsize * log.grocery + famsize * log.eatout + famsize * log.delivery
, data=df_adult, method="lm", preProcess="scale", trControl=train_control)

print("model_2 cv results:")
print(m2_cv$results)
